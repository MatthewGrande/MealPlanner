...
public class User {
	...
	public User(int aGoalCalorie, MealPlannerApp aApplication) {
		...
		if (!didAddApplication) {
			throw new RuntimeException(
					"Unable to create user due to application. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
		}
	...
	public boolean addDietTypeAt(DietType aDietType, int index) {
		...
		if (addDietType(aDietType)) {
			...
			if (index > numberOfDietType()) {
				index = numberOfDietType() - 1;
			}
			dietType.remove(aDietType);
	...
	public boolean addPastDayAt(Day aPastDay, int index) {
		...
		if (addPastDay(aPastDay)) {
			...
			if (index > numberOfPastDays()) {
				index = numberOfPastDays() - 1;
			}
			pastDays.remove(aPastDay);
	...
	public boolean addOwnedIngredientAt(OwnedIngredient aOwnedIngredient, int index) {
		...
		if (addOwnedIngredient(aOwnedIngredient)) {
			...
			if (index > numberOfOwnedIngredients()) {
				index = numberOfOwnedIngredients() - 1;
			}
			ownedIngredients.remove(aOwnedIngredient);
	...
	public String toString() {
		return super.toString() + "[" + "goalCalorie" + ":" + getGoalCalorie() + "]"
				+ System.getProperties().getProperty("line.separator") + "  " + "application = "
				+ (getApplication() != null ? Integer.toHexString(System.identityHashCode(getApplication())) : "null")
				+ System.getProperties().getProperty("line.separator") + "  " + "currentDay = "
				+ (getCurrentDay() != null ? Integer.toHexString(System.identityHashCode(getCurrentDay())) : "null");
	}
}